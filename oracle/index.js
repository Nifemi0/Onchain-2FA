...
// prepared statements
const insertUserStmt = db.prepare('INSERT OR REPLACE INTO users (userId, secret_enc, createdAt) VALUES (?, ?, ?)');
const getUserStmt = db.prepare('SELECT secret_enc FROM users WHERE userId = ?');
...
app.post('/submit-code', express.json(), (req, res) => {
  try {
    // raw body HMAC verification — express.json already parsed; re-create raw payload canonical form
    const raw = JSON.stringify(req.body);
    const sig = req.header('x-hmac-signature');
    if (!verifyHmac(Buffer.from(raw, 'utf8'), sig)) {
...
app.post('/admin/add-user', express.json(), (req, res) => {
  const { userId, seed } = req.body;
  if (!userId || !seed) return res.status(400).send({ ok: false, error: 'bad_payload' });
  const enc = encryptSeed(seed);
  const now = Math.floor(Date.now() / 1000);
  insertUserStmt.run(userId, enc, now);
...
async function processRequest(requestId, userId, expiryAt) {
  ...
  // For now, assume trap is not triggered and use a placeholder blockHash.
  // In a real scenario, the oracle would query the chain for the blockHash
  // and potentially a trap contract for the trap state.
  const trapTriggered = false; 
