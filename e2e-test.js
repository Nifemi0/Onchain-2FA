...
describe('Full Integration Test', () => {
    before(async function() {
        ...
        // 1. Start Oracle (Temporarily disabled for frontend/backend testing)
        // console.log('Starting Oracle...');
        oracleProcess = spawn('/usr/bin/node', ['index.js'], { cwd: ORACLE_DIR, detached: true, stdio: 'inherit', env: { ...process.env, PATH: '/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin' } });
        // oracleProcess.unref();
        ...
        backendProcess = spawn('/usr/bin/npm', ['start'], { cwd: BACKEND_DIR, detached: true, stdio: 'inherit', env: { ...process.env, PATH: '/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin' } });
        ...
        frontendProcess = spawn('/usr/local/bin/http-server', ['.', '-p', FRONTEND_PORT, '-c-1'], { cwd: FRONTEND_DIR, detached: true, stdio: 'inherit', env: { ...process.env, PATH: '/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin' } });
    ...
    after(async () => {
        ...
        // if (oracleProcess) process.kill(-oracleProcess.pid); // Kill process group (Temporarily disabled)
        if (backendProcess && backendProcess.pid) process.kill(-backendProcess.pid);
        if (frontendProcess && frontendProcess.pid) process.kill(-frontendProcess.pid);
